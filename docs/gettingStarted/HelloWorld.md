...
<img src="https://tc-cdn.processon.com/po/684fa7adbc9bf7627b3e562d-695364f703b6d634b3327d18" style="max-width: 100%; width: 60%;" alt="计数器组件">
...
通过「KWC」配套脚手架生成的组件项目，会自动生成上述文件，这些文件存放于 `src/modules/app/` 目录下，包含 _app.html_, _app.css_ 和 *app.js*三个文件。

| 文件       | 必需 | 说明                               |
| :--------- | :--- | :--------------------------------- |
| _app.html_ | 是   | 定义组件的 HTML 视图结构。         |
| _app.css_  | 是   | 定义组件的 CSS 样式。              |
| _app.js_   | 否   | 定义组件的 JavaScript 逻辑与行为。 |

:::info
关于组件结构、命名空间和元数据文件的完整说明，请参阅《开发指南》中的 [创建组件](https://dev.kingdee.com/kwc/development-guide/define-component) 。
:::
...
1. 在 VSCode 左侧展开文件路径 `workspace/mycontrol/src/modules/app` 。
2. 选择并打开 _app.html_ 文件。
3. 替换如下的代码内容：
   ```html
   <template>
...
<img src="https://tc-cdn.processon.com/po/684fa7adbc9bf7627b3e562d-695368c39c0db14c9eaa7fc5" style="max-width: 100%; width: 60%;" alt="计数器组件">
...
在下一章中，请在 [综合实践](https://dev.kingdee.com/kwc/getting-started/todo-list/)，探索如何使用多个「KWC」组件构建一个功能完整的待办事项应用。
