...
order: 0
glossary: Git | Node | 脚手架 | 包管理 | Visual Studio Code | VSCode
---

# 开发准备

欢迎使用「KWC」(Kingdee Web Components) 进行前端组件开发！<br>
在开始使用「KWC」之前，你需要配置相应的开发环境并创建组件项目，本文档将指导你完成。<br>
...
为了获得最佳的「KWC」开发体验，你需要准备好代码编辑器、Git 和 Node.js 运行环境，并完成包管理和脚手架安装。

### 代码编辑器

准备代码编辑器，推荐使用 Visual Studio Code (VSCode)。

1. **下载安装**：前往 [VScode 官网](https://code.visualstudio.com/) 下载最新版并完成安装。
2. **创建项目文件夹**：选择合适磁盘创建项目文件夹，例如 _workspace_，下述示例基于该文件夹。

   :::info

   - 你可以自定义项目文件夹名称，需注意该名称只能包含小写字母和数字，不能包含空格和特殊字符;
   - _workspace_ 将作为你所有「KWC」组件项目的工作区根目录，后续创建的每个组件项目都会生成在此文件夹下。
     :::

3. **打开终端**：打开 VSCode，选择 _workspace_ 文件夹，使用快捷键（Ctrl + `）打开 VSCode 内置终端，用于完成下面的命令行操作。<br>
   <img src="https://tc-cdn.processon.com/po/684fa7adbc9bf7627b3e562d-69534669c5602355134d81a2" style="max-width: 100%; width: 60%;" alt="VSCode 工作区">

### Git 环境

安装并配置 Git 环境。

1. **下载安装**：前往 [Git 官网](https://git-scm.com/) 下载并完成安装。
...
3. **执行 Git 命令**：打开文件资源管理器，进入 _workspace_ 文件夹，右键选择 **Open Git Bash here** 即可在该目录下执行 Git 命令。<br>
   <img src="https://tc-cdn.processon.com/po/684fa7adbc9bf7627b3e562d-695c7cd903b6d634b332e99c" style="max-width: 100%; width: 60%;" alt="Git 环境">
...
2. **确认成功**：输入`pnpm -v`命令，回车返回版本表示安装成功。

### 安装脚手架

在完成环境配置和包管理工具安装之后，你可以通过「KWC」配套的脚手架工具，一键生成标准化的「KWC」项目模板，快速开始组件开发。
根据以下步骤完成「KWC」脚手架安装：
...
:::success
此命令将 `kd-custom-control-cli` 工具安装到全局环境，使您可以在任何目录下使用 `kd-custom-control-cli` 命令。
:::
...
1. **创建组件名称**：在 VSCode 内置终端，输入`kd-custom-control-cli create mycontrol`命令，创建一个名为 `mycontrol` 的「KWC」项目。
...
3. **确认成功**：当终端界面提示如下信息时，表示组件项目 `mycontrol` 创建成功。此时 VSCode 左侧 _workspace_ 成功创建 `mycontrol` 组件项目。
...
<img src="https://tc-cdn.processon.com/po/684fa7adbc9bf7627b3e562d-695363059c0db14c9eaa7f36" style="max-width: 100%; width: 60%;" alt="启动 KWC">
...
<img src="https://tc-cdn.processon.com/po/684fa7adbc9bf7627b3e562d-69536407df09ee077deb7f81" style="max-width: 100%; width: 60%;" alt="页面预览">
...
在下一章中，我们将直接进入实战环节。通过一个经典的 [Hello World](https://dev.kingdee.com/kwc/getting-started/hellow-world) 快速入门案例，带你使用「KWC」基础组件编写你的第一个交互功能，让你亲身体验「KWC」开发的高效与便捷。
